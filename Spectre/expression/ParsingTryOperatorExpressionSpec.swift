...
func specTryOperatorExpression() {
  let parser = Parser()
    ...
    $0.it("should return a try operator expression") {
      ...
      for (testString, testKind) in testStrings {
        ...
        guard let tryOpExpr = try? parser.parseTryOperatorExpression() else {
          throw failure("Failed in getting a try operator expression for code `\(testCode)`.")
        }
        try expect(tryOpExpr.kind) == testKind
        try expect(tryOpExpr.expression is IdentifierExpression).to.beTrue()
      }
    }
  }
}
